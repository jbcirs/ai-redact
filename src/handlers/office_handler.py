...
_PAGE_H = 792.0
_MARGIN = 50.0
_FONT = "helv"  # plain built-in font (Latin-1 repertoire)


# ---------------------------------------------------------------------------
# Simplified-PDF writer: top-to-bottom text flow with wrap + page breaks
# ---------------------------------------------------------------------------
...
class _PdfFlow:
    ...
    def new_page(self) -> None:
        ...
        self.y = _MARGIN  # next baseline is placed at y + leading
    ...
    def _wrap(self, text: str, size: float) -> list[str]:
        ...
        for raw in text.split("\n"):
            ...
            for word in raw.split(" "):
                ...
                while (
                    fitz.get_text_length(word, fontname=_FONT, fontsize=size)
                    > max_w
                ):
                    if cur:
                        lines.append(cur)
                    ...
                    while k > 1 and (
                        fitz.get_text_length(
                            word[:k], fontname=_FONT, fontsize=size
                        )
                        > max_w
                    ):
                        k -= 1
                ...
                if (
                    fitz.get_text_length(cand, fontname=_FONT, fontsize=size)
                    <= max_w
                ):
                    cur = cand
                else:
    ...
    def add_text(self, text: str, size: float = 11.0) -> None:
        ...
        for line in self._wrap(text, size):
            if self.y + leading > _PAGE_H - _MARGIN:
                self.new_page()
            self.y += leading
            if line:
                self.page.insert_text(
                    (_MARGIN, self.y), line, fontsize=size, fontname=_FONT
                )

    def add_gap(self, points: float = 6.0) -> None:
        self.y = min(self.y + points, _PAGE_H - _MARGIN)
    ...
    def add_image(self, blob: bytes) -> bool:
        ...
        if h > max_h:
            w *= max_h / h
            h = max_h
        if self.y + h > _PAGE_H - _MARGIN:
            self.new_page()
        rect = fitz.Rect(_MARGIN, self.y, _MARGIN + w, self.y + h)
        try:
