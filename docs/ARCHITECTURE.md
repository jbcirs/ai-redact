...
├── output/                 # redacted files; EMPTIED at the start of every batch run
│   └── logs/               # every *_report.txt lives here, not beside the files
...
(`0********56`) so the report is safe even if it strays from the
redacted file; dry-run reports show full values for accuracy checking.

Reports live in a `logs/` subfolder next to the output file
(`default_report_path()` in `src/redact.py`, used by every write path:
`write_report`, `write_native_report`, and `combine_outputs.py`), keyed
on the **full output filename including its extension**, not `.stem` —
`.stem` only strips the final extension, so `foo_redacted.txt` and
`foo_redacted.yaml` (same stem, different native-format outputs) would
otherwise collide on one `logs/foo_redacted_report.txt` and silently
overwrite each other's audit trail. `combine_outputs.py`'s refuse-to-merge check (`src/combine_outputs.py`,
run via `--combine` / `output.combine: true`) depends on finding each
file's own report at this exact path — a file with no PASSing report at
`logs/<name>_report.txt` blocks the merge. An explicit `--report PATH`
bypasses the `logs/` subfolder entirely and is honored exactly as given.
...
6. Empty `output/` (keeps `.gitkeep`), then recreate `output/logs/`.
7. Run `src/redact.py` per file → `output/<name>_redacted.<ext>` with its
   report in `output/logs/`; classify per-file exit codes (0 ok, 2
   unreadable, 3 verify failed, 4 unsupported, 5 encrypted, else failure).
